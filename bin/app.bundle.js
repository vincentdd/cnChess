    ...
    */
    ...
    function SuperKnight() {
        ...
        this.checkOrder = function () {
            if (gameManager.act === "transverse") return false;else return true;
        };
        //士、象的计算目标坐标方法，Knight对象继承此实例是，自身所有的同名方法会覆盖此方法
        this.getPoint = function (num) {
            var temp = gameManager.step,
                sign = "";
        ...
        this.isViable = function () {
            ...
            var roots = arr.map(function (x) {
                var temp = _that.toStringWithSign(x); //获取x的带符号string格式
        ...
        **********************************************/
        ...
        this.isRoadClear = function (startX, startY, endX, endY) {
            ...
            if (startX === endX) {
                ...
                for (var i = 0, len = soldierArr.length; i < len; i++) {
                    if (soldierArr[i].color === gameManager.color) {
                        //根据棋子颜色转换坐标值
                        X = soldierArr[i].posX;
                        Y = soldierArr[i].posY;
                    } else {
                    ...
                    if (X === startX && Y > min && Y < max) count += 1;
                }
            } else {
                //水平移动
                min = Math.min(startX, endX);
                max = Math.max(startX, endX);
                for (var _i = 0, _len = soldierArr.length; _i < _len; _i++) {
                    if (soldierArr[_i].color === gameManager.color) {
                        //根据棋子颜色转换坐标值
                        X = soldierArr[_i].posX;
                        Y = soldierArr[_i].posY;
                    } else {
                        X = 500 - soldierArr[_i].posX;
                        Y = 550 - soldierArr[_i].posY;
                    }
                    if (Y === startY && X > min && X < max) count += 1;
                }
            }
            return count;
        };
    ...
    function RulesOfGuard() {
        this.rules = function () {
            ...
            if (!this.checkOrder()) //检测命令是否为平
                return false;
            this.getPoint(step);
            if (gameManager.posX < 200 || gameManager.posX > 300 || gameManager.posY < 50 || gameManager.posY > 150) return false;
    ...
    function RulesOfBishop() {
        this.rules = function () {
            ...
            if (!this.checkOrder()) //检测命令是否为平
                return false;
            this.getPoint(step);
            if (gameManager.posX - this.posX != 100 || gameManager.posY > 250) return false;
            feetPosition = this.isViable(); //检查象眼位置
            if (!feetPosition) return false;
            return gameManager.checkPoint(gameManager.posX, gameManager.posY, gameManager.color);
        };
    }
    //马移动规则
    function RulesOfKnight() {
        ...
        this.rules = function () {
            ...
            /*if (!this.getPoint()) //校验步长是否符合规则
                return false;*/
            this.getPoint();
            if (Math.abs(gameManager.posX - this.posX) != 50 && Math.abs(gameManager.posX - this.posX != 100)) return false;
    ...
    function RulesOfKing() {
        ...
        this.rules = function () {
            ...
            if (gameManager.posX < 200 || gameManager.posX > 300 || gameManager.posY < 50 || gameManager.posY > 150) return false;
            if (sum != 50 || targetPosition === false) return false;else return true;
        };
    }

    function RulesOfCannon() {
        //炮的父类，定义炮的行走规则
        var chessmanOnTheRoad = void 0,
            targetPosition = void 0;
        ...
        this.rules = function () {
            ...
            if (typeof targetPosition === "boolean" && targetPosition === true && chessmanOnTheRoad === 0) return targetPosition;
            if (typeof targetPosition != "boolean" && chessmanOnTheRoad === 1) return targetPosition;else return false;
        };
    }

    function RulesOfRook() {
        //車、车的父类，定义車的行走规则
        var result = void 0;
        this.rules = function () {
            this.move.call(gameManager);
            this.isRoadClear(this.posX, this.posY, gameManager.posX, gameManager.posY) === 0 ? result = true : result = false;
            if (result) return gameManager.checkPoint(gameManager.posX, gameManager.posY, gameManager.color);else return result;
        };
    }

    function RulesOfSoldier() {
        //兵、卒的父类，定义卒的行走规则
        var temp = void 0;
        this.rules = function () {
            if (gameManager.act === "transverse") {
                temp = Math.abs(this.posX - gameManager.step); //步长
                if (this.posY < 300 || temp != 50) return false;
            } else if (gameManager.act === "forward") {
                if (gameManager.step != 50 || gameManager.posY === "-") return false;
            }
            this.move.call(gameManager);
            return gameManager.checkPoint(gameManager.posX, gameManager.posY, gameManager.color);
        };
    ...
    var gameManager = {
        ...
        getOrder: function getOrder(str) {
            ...
            if (result === false) {
                ...
                if (typeof result === "number") _that.dead(result);
                obj.move();
        ...
        getStep: function getStep(arr, str) {
            for (var key in arr) {
                if (key.indexOf(str) != -1) return arr[key];
            }
        },
        handleOrder: function handleOrder(arr) {
            //处理输入命令
            var reg = /(\u524d)|(\u540e)/,
                temp = void 0,
                val = void 0,
        ...
        pickUpChessman: function pickUpChessman(arr) {
            //根据命令寻找棋子，返回soldierArr中的棋子对象或者false
            var temp = [],
                result = void 0;
        ...
        checkPoint: function checkPoint(X, Y, color) {
            var result = void 0,
                difColorPosX = 500 - X,
                //转换坐标到异颜色棋子坐标系
            difColorPosY = 550 - Y;
            //console.log(difColorPosX + " , " + difColorPosY);
            if (X < 50 || X > 450 || Y < 50 || Y > 500) return false;
            for (var i = 0, len = soldierArr.length; i < len; i++) {
        ...
        addHistroy: function addHistroy() {
            //将当前的棋子列表添加到histroy属性
            var temp = void 0,
                obj = void 0,
                arr = [],
            ...
            if (this.count === len) {
                ...
                //当游标位置在数组中间时，删除位于游标之后的部分并添加当前最新的棋子
                temp = len - this.count;
        ...
        dead: function dead(num) {
            soldierArr.splice(num, 1);
        },
        showHistroy: function showHistroy(num) {
    ...
    var objCanvas = {
        ...
        //绘制画布
        painting: function painting(obj) {
        ...
        //循环棋子队列
        loopArr: function loopArr(arr) {
            for (var i = 0; i < arr.length; i++) {
                this.soldierDrawing(arr[i]);
            }
        },
        //绘制棋子
        soldierDrawing: function soldierDrawing(obj) {
            var radian = obj.color === 1 ? Math.PI : 0,
        ...
        //画棋盘线方法
        lineDrawing: function lineDrawing(mx, my, lx, ly) {
            this.ctx.beginPath();
        ...
        drawFont: function drawFont(str, fontSize, color, startX, startY, radian, offsetX, offsetY) {
